...
class Cart {
	...
	public function getProducts() {
		if (!$this->data) {
			foreach ($this->session->data['cart'] as $key => $quantity) {
				...
				//TecDoc module
				if(is_array($quantity) AND $quantity['tecdoc']=="Y"){
					$arTDMCart = $quantity;
					$this->data[$key] = array(
						'key'          => $key,
						'product_id'   => 0,
						'name'         => $arTDMCart['name'],
						'model'        => $arTDMCart['brand'],
						'shipping'     => 1,
						'image'        => $arTDMCart['image'],
						'quantity'     => $arTDMCart['quantity'],
						'stock'        => $arTDMCart['stock'],
						'option'       => $arTDMCart['option'],
						'minimum'	   => $arTDMCart['minimum'],
						'price'        => $arTDMCart['price'],
						'total'        => ($arTDMCart['price'] * $arTDMCart['quantity']),		
						'tax_class_id' => false,
						'download'	   => false,
						'reward'	   => false, 
						'points'	   => $arTDMCart['points'],
						'points_prefix'=> $arTDMCart['points_prefix'],
						'weight'	   => $arTDMCart['weight'],
						'weight_class_id'=>$arTDMCart['weight_class_id'],
						'weight_prefix'=> $arTDMCart['weight_prefix']
					);
					
					$this->data[$key]['tecdoc'] = "Y";
					$this->data[$key]['product_url'] = $arTDMCart['product_url'];
					continue;
				}
				////////////////////////////////////////////////////
	...
	public function update($key, $qty) {
		if ((int)$qty && ((int)$qty > 0)) {
			//TecDoc module
			if(is_array($this->session->data['cart'][$key])){
